...
def saveToHub():
    try:
        ...
        progressDialog.show("Save To Hub", "Uploading to Autodesk Hub...", 0, 100)
        
        try:
            # Implement the upload logic with the token
            ...
            if success:
                ...
                ui.messageBox(f"Failed to upload {model_name}. See logs for details.",
                             "SaveToHub", 
                             adsk.core.MessageBoxButtonTypes.OKButtonType,
    ...
    def upload_file_to_hub(file_path, project_id, item_id, access_token):
        """Upload a file to Autodesk Hub using the token from C#"""
        try:
            import requests
            ...
            logging.info(f"Starting upload of {file_name} (size: {file_size}) to project {project_id}, item {item_id}")
        
            # Create the version data
            version_url = f"https://developer.api.autodesk.com/data/v1/projects/{project_id}/items/{item_id}/versions"
            headers = {
            ...
            # Create the version (placeholder for the file)
            response = requests.post(version_url, headers=headers, json=data)
        
            if response.status_code not in [200, 201]:
                logging.error(f"Failed to create version: {response.status_code} - {response.text}")
                return False
        
            # Get upload URL from the response
            version_response = response.json()
            logging.info(f"Version created: {version_response}")
        
            upload_url = None
            for included in version_response.get("included", []):
                if included.get("type") == "objects":
                    if "relationships" in included and "self" in included["relationships"]:
                        upload_url = included["relationships"]["self"]["meta"]["link"]["href"]
                        break
        
            if not upload_url:
                logging.error("Failed to get upload URL from response")
                return False
            ...
            with open(file_path, 'rb') as file_content:
                ...
                logging.info(f"Uploading file content ({file_size} bytes)")
